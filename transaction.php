...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_transaction_id'])) {
    $delId = intval($_POST['delete_transaction_id']);
    if ($delId > 0) {
...
$transactions = [];
$result = $conn->query("SELECT id, transaction_date, meetup_place, book, buyer, seller_email, amount, status, created_at FROM transactions ORDER BY created_at DESC");
if ($result) {
...
<head>
    ...
    <title>Transactions - UEsed Books</title>
    ...
    <style>
        ...
        /* ── FIX: Rejected → red badge (distinct from cancelled) ──────── */
        .status-rejected {
            background: #fdeaea;
            color: #c0392b;
...
<body class="admin-body">
    ...
    <div class="admin-wrapper">
        ...
        <aside class="admin-sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="transaction.php" class="sidebar-link active" data-section="transaction">
        ...
        <main class="admin-main">
            <!-- Search Bar + Add Sale + Filter + Export -->
            <div class="users-search-bar">
                <div class="users-search-wrap">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                    <input type="text" class="users-search-input" id="txnSearch" placeholder="Search Books, Buyer or Seller...">
                </div>
                ...
                <div class="users-filter-dropdown">
                    ...
                    <ul class="filter-dropdown-list" id="alphaFilterList">
                        <li class="filter-dropdown-item active" data-sort="newest">Newest</li>
                        <li class="filter-dropdown-item" data-sort="oldest">Oldest</li>
                        <li class="filter-dropdown-item" data-sort="az">A-Z (Book)</li>
                        <li class="filter-dropdown-item" data-sort="za">Z-A (Book)</li>
                        <li class="filter-dropdown-item" data-sort="amount-high">Highest Amount</li>
                        <li class="filter-dropdown-item" data-sort="amount-low">Lowest Amount</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="users-table-wrap">
                <table class="users-table" id="txnTable">
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>Meet-up Place</th>
                    ...
                    <tbody>
                        <?php if (count($transactions) > 0): ?>
                            ...
                            <tr data-created="<?php echo htmlspecialchars($txn['created_at']); ?>" data-amount="<?php echo (float)$txn['amount']; ?>">
                                <td class="date-cell"><?php echo htmlspecialchars(date('M d, Y', strtotime($txn['transaction_date']))); ?></td>
                                <td><?php echo htmlspecialchars($txn['meetup_place']); ?></td>
                                <td class="user-name"><?php echo htmlspecialchars($txn['book']); ?></td>
                                <td><?php echo htmlspecialchars($txn['buyer']); ?></td>
                                <td><?php echo htmlspecialchars($txn['seller_email']); ?></td>
                                <td class="amount-cell">₱<?php echo number_format((float)$txn['amount'], 2); ?></td>
                                <td>
                                    <?php
                                    $statusLower = strtolower($txn['status']);
                                    $statusClass = 'status-paid';
                                    if ($statusLower === 'pending')    $statusClass = 'status-pending';
                                    elseif ($statusLower === 'processing') $statusClass = 'status-processing';
                                    elseif ($statusLower === 'cancelled')  $statusClass = 'status-cancelled';
                                    elseif ($statusLower === 'on hold')    $statusClass = 'status-on-hold';
                                    elseif ($statusLower === 'rejected')   $statusClass = 'status-rejected'; // ← FIX
                                    ?>
                                    <span class="status-badge <?php echo $statusClass; ?>"><?php echo htmlspecialchars($txn['status']); ?></span>
                                </td>
                                <td class="row-actions">
                                    <button class="row-delete-btn" onclick="confirmDeleteTxn(<?php echo (int)$txn['id']; ?>, '<?php echo htmlspecialchars(addslashes($txn['book'])); ?>')" title="Delete transaction">
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="8" style="text-align:center;color:#888;padding:2rem;">No transactions recorded yet.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
    ...
    <div class="confirm-modal-overlay" id="deleteModal">
        <div class="confirm-modal">
            ...
            <h3 class="confirm-modal-title">Delete this transaction?</h3>
            ...
            <form method="POST" action="transaction.php" id="deleteTxnForm" style="display:none;">
                <input type="hidden" name="delete_transaction_id" id="deleteTxnId" value="">
            </form>
        </div>
    </div>
    ...
    <div class="confirm-modal-overlay" id="deleteSuccessModal" <?php if ($deleteSuccess): ?>style="opacity:1;visibility:visible;"<?php endif; ?>>
        <div class="confirm-modal" <?php if ($deleteSuccess): ?>style="transform:translateY(0) scale(1);opacity:1;"<?php endif; ?>>
            ...
            <h3 class="confirm-modal-title">Transaction Deleted</h3>
            <p class="confirm-modal-msg">The transaction has been permanently removed.</p>
            <div class="confirm-modal-actions">
    ...
    <script>
        ...
        function confirmDeleteTxn(txnId, bookTitle) {
            var modal = document.getElementById('deleteModal');
            document.getElementById('deleteModalMsg').textContent = 'Are you sure you want to delete the transaction for "' + bookTitle + '"? This action cannot be undone.';
        ...
        if (alphaBtn && alphaList) {
            ...
            filterItems.forEach(function(item) {
                item.addEventListener('click', function() {
                    ...
                    var labels = { 'newest': 'Newest', 'oldest': 'Oldest', 'az': 'A-Z', 'za': 'Z-A', 'amount-high': 'Highest', 'amount-low': 'Lowest' };
                    alphaBtn.childNodes.forEach(function(n) {
                        if (n.nodeType === 3 && n.textContent.trim()) n.textContent = ' ' + (labels[sort] || 'Filter') + ' ';
                    });
        ...
        function sortTable(mode) {
            ...
            rows.sort(function(a, b) {
                ...
                if (mode === 'amount-high' || mode === 'amount-low') {
                    var amtA = parseFloat(a.dataset.amount || 0);
                    var amtB = parseFloat(b.dataset.amount || 0);
                    return mode === 'amount-high' ? amtB - amtA : amtA - amtB;
                }
                return 0;
            });
